...
class AwsInvoke {
  ...
  readPayload() {
    let optionsFileContent = this.serverless.utils
        .readFileSync(path.join(this.serverless.config.servicePath, this.options.path));

    try {
      return new Buffer(yaml.safeLoad(optionsFileContent));
    } catch(ex) {
      return new Buffer(JSON.stringify(this.options.data || {}))
    }
  }
  ...
  extendedValidate() {
    ...
    if (this.options.path) {
      ...
      this.options.data = this.readPayload();
    }
